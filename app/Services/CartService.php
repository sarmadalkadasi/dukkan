...
class CartService
{
    ...
    {
        // We need to put this in try-catch, otherwise if something goes
    ...
    public function updateItemQuantityInCookies(int $productId, int $quantity, array $optionIds): void
    {
        ...
        // Use a unique based on product ID and option IDs
    ...
    protected function saveItemToDatabase(int $productId, int $quantity, $price, array $optionIds): void
    {
        ...
        $cartItem = CartItem::where('user_id', $userId)
            ->where('product_id', $productId)
            ->where('variation_option_ids', json_encode($optionIds))
            ->first();
